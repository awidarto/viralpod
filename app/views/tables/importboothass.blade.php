...
{{$form->open('import/commitboothass/'.$importid,'POST',array('class'=>'custom','id'=>'commit_form'))}}
			...
			{{ $form->hidden('exhibitorid',$exhibitordata['_id'])}}
			{{ $form->hidden('companyemail',$exhibitordata['email'])}}
            {{ $form->hidden('companyname',$exhibitordata['company'])}}
            {{ $form->hidden('boothname',$exhibitordata['booth'])}}
            {{ $form->hidden('hallname',$exhibitordata['hall'])}}
            {{ $form->hidden('companypic',$exhibitordata['firstname'].' '.$exhibitordata['lastname']) }}
     ...
     <div class="row">
        <div class="span6 align-left">
           ...
           <hr class="win-command" />
			...
			@if(isset($doneurl) && $doneurl != '')
				<a class="win-command" href="{{URL::to($doneurl)}}">
					<span class="win-commandimage win-commandring">&#xe1ca;</span>
					<span class="win-label">Done</span>
				</a>
			@endif
